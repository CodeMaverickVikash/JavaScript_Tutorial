...
// How everthing working inside the browser

console.log("start");

// once below line comes into picture, it will attach callback with timer of 5000ms into Web APIs environment - Time is the API of browser.
setTimeout(() => {
...
/*
 * Why we need callback queue - when user click continuosly on element, in that case multiple callback trigger and goes into callback queue, if user has clicked 5 times then 5 callback will go into callback queue and by event loop pushed it into callstack one by one.
 */
 ...
 * Read it out STARVATION - When microtask queue has tree of task then any callback or not going to execute from callback queue, in this scenarios starvation problem created.
 */

// JS Engine EXPOSED 🔥 Google's V8 Architecture - Akshay Saini
